...
"""
Compiler classes for Wenet backend
"""
...
from wenet_active_grammar import NativeWFST, WenetRule
from wenet_active_grammar import Compiler as WenetAGCompiler
...
class WenetCompiler(CompilerBase, WenetAGCompiler):

    def __init__(self, model_dir, auto_add_to_user_lexicon=None, allow_online_pronunciations=None, **kwargs):
        CompilerBase.__init__(self)
        WenetAGCompiler.__init__(self, model_dir=model_dir, **kwargs)
        ...
        self.wenet_rule_by_rule_dict = collections.OrderedDict()  # Rule -> WenetRule
        self.wenet_rules_by_listreflist_dict = collections.defaultdict(set)  # Rule -> Set[WenetRule]
        self.internal_grammar = InternalGrammar('!wenet_engine_internal')

    impossible_word = property(lambda self: self._longest_word.lower())  # FIXME
    unknown_word = property(lambda self: self._oov_word)

    #-----------------------------------------------------------------------
    # Methods for handling lexicon.
    ...
    def compile_grammar(self, grammar, engine):
        ...
        wenet_rule_by_rule_dict = collections.OrderedDict()  # Rule -> WenetRule
        for rule in grammar.rules:
            if rule.exported:
                ...
                wenet_rule.parent_grammar = grammar
                wenet_rule.parent_rule = rule
                wenet_rule_by_rule_dict[rule] = wenet_rule

                try:
                    self._compile_rule_root(rule, grammar, wenet_rule)
                    wenet_rule.has_dictation = bool(wenet_rule.has_dictation)  # Convert None to False
                except Exception:
                    raise self.make_compiler_error_for_wenet_rule(wenet_rule)

        self.wenet_rule_by_rule_dict.update(wenet_rule_by_rule_dict)
        return wenet_rule_by_rule_dict

    def _compile_rule_root(self, rule, grammar, wenet_rule):
        src_state, dst_state = self._compile_rule(rule, grammar, wenet_rule, wenet_rule.fst, export=True)
        if wenet_rule.fst.native and not wenet_rule.fst.has_path():
            ...
            self._compile_impossible(None, src_state, dst_state, grammar, wenet_rule, wenet_rule.fst)
        wenet_rule.compile(lazy=self.lazy_compilation)

    def _compile_rule(self, rule, grammar, wenet_rule, fst, export):
        ...
        self.compile_element(rule.element, inner_src_state, dst_state, grammar, wenet_rule, fst)
    ...
    def unload_grammar(self, grammar, rules, engine):
        for rule in rules:
            wenet_rule = self.wenet_rule_by_rule_dict[rule]
            # Unload wenet_rule: destroy() handles WenetAGCompiler stuff; we must handle ours
            wenet_rule.destroy()
            del self.wenet_rule_by_rule_dict[rule]
            for wenet_rules_set in self.wenet_rules_by_listreflist_dict.values():
                wenet_rules_set.discard(wenet_rule)
            # NOTE: the wenet_rule_by_rule_dict we returned from compile_grammar() is not updated, but it should be dropped upon unload anyway!

    def update_list(self, lst, grammar):
        # Note: we update all rules in all grammars that reference this list (unlike WSR/natlink?)
        lst_wenet_rules = self.wenet_rules_by_listreflist_dict[id(lst)]
        for wenet_rule in lst_wenet_rules:
            with wenet_rule.reload():
                self._compile_rule_root(wenet_rule.parent_rule, grammar, wenet_rule)
    ...
    # @trace_compile
    def _compile_sequence(self, element, src_state, dst_state, grammar, wenet_rule, fst):
        ...
        if len(children) == 0:
            ...
            return self.compile_element(children[0], src_state, dst_state, grammar, wenet_rule, fst)
            ...
            if is_repetition and element.optimize:
                ...
                self.compile_element(children[0], s1, s2, grammar, wenet_rule, fst)
                if not fst.has_eps_path(s1, s2, self._eps_like_nonterms):
                    ...
                    for i, child in enumerate(children[1:], start=1):
                        s1 = states[i]
                        s2 = states[i + 1]
                        self.compile_element(child, s1, s2, grammar, wenet_rule, fst)
                    return
    ...
    # @trace_compile
    def _compile_alternative(self, element, src_state, dst_state, grammar, wenet_rule, fst):
        src_state = self.add_weight_linkage(src_state, dst_state, self.get_weight(element), fst)
        for child in element.children:
            self.compile_element(child, src_state, dst_state, grammar, wenet_rule, fst)

    # @trace_compile
    def _compile_optional(self, element, src_state, dst_state, grammar, wenet_rule, fst):
        src_state = self.add_weight_linkage(src_state, dst_state, self.get_weight(element), fst)
        self.compile_element(element.children[0], src_state, dst_state, grammar, wenet_rule, fst)
        fst.add_arc(src_state, dst_state, None)

    # @trace_compile
    def _compile_literal(self, element, src_state, dst_state, grammar, wenet_rule, fst):
    ...
    # @trace_compile
    def _compile_rule_ref(self, element, src_state, dst_state, grammar, wenet_rule, fst):
        weight = self.get_weight(element)  # Handle weight internally below without adding a state
        # Compile target rule "inline"
        rule_src_state, rule_dst_state = self._compile_rule(element.rule, grammar, wenet_rule, fst, export=False)
        fst.add_arc(src_state, rule_src_state, None, weight=weight)
        fst.add_arc(rule_dst_state, dst_state, None)

    # @trace_compile
    def _compile_list_ref(self, element, src_state, dst_state, grammar, wenet_rule, fst):
        src_state = self.add_weight_linkage(src_state, dst_state, self.get_weight(element), fst)
        ...
        self.wenet_rules_by_listreflist_dict[id(element.list)].add(wenet_rule)
        for child_str in element.list.get_list_items():
            self._compile_literal(MockLiteral(child_str.split()), src_state, dst_state, grammar, wenet_rule, fst)

    # @trace_compile
    def _compile_dictation(self, element, src_state, dst_state, grammar, wenet_rule, fst):
        wenet_rule.has_dictation = True
        src_state = self.add_weight_linkage(src_state, dst_state, self.get_weight(element), fst)
        # fst.add_arc(src_state, dst_state, '#nonterm:dictation', olabel=NativeWFST.eps)
        ...
        fst.add_arc(extra_state, dst_state, NativeWFST.eps, '#nonterm:end')
        # fst.add_arc(extra_state, dst_state, '!SIL', '#nonterm:end')  # Causes problems with lack of phones during decoding

    # @trace_compile
    def _compile_impossible(self, element, src_state, dst_state, grammar, wenet_rule, fst):
    ...
    # @trace_compile
    def _compile_empty(self, element, src_state, dst_state, grammar, wenet_rule, fst):
        src_state = self.add_weight_linkage(src_state, dst_state, self.get_weight(element), fst)
        fst.add_arc(src_state, dst_state, NativeWFST.eps)
    ...
    def make_compiler_error_for_wenet_rule(self, wenet_rule):
        message = "Exception while compiling %s in %s" % (wenet_rule.parent_rule, wenet_rule.parent_grammar)
        if six.PY2: self._log.exception("%s: %s", self, message)  # Imitate PY3's chained exceptions traceback
        rule_tree = wenet_rule.parent_rule.element.element_tree_string()
        ...
        if not self._log.isEnabledFor(10) and rule_tree.count('\n') >= 10:
            ...
            rule_tree_lines.append("    logging.getLogger('wenet.compiler').setLevel(logging.DEBUG)")
