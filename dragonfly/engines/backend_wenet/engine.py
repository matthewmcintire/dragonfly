...
from wenet_active_grammar       import WenetError, WenetRule
...
from .recobs                    import WenetRecObsManager
from .testing                   import debug_timer
from dragonfly.grammar.state    import State
from dragonfly.windows          import Window

# Import the Wenet compiler class. Suppress metaclass TypeErrors raised
# during documentation builds caused by mocking WenetAG.
try:
    from .compiler                  import WenetCompiler
except TypeError:
...
class WenetEngine(EngineBase, DelegateTimerManagerInterface):
    """ Speech recognition engine back-end for Wenet recognizer. """

    _name = "wenet"
    DictationContainer = DictationContainerBase
    _required_wag_version = "0.1.0"
    ...
    def __init__(self, model_dir=None, input_device_index=None,
        ...
        try:
            import wenet_active_grammar, sounddevice, webrtcvad
        except ImportError as e:
            self._log.error("%s: Failed to import Wenet engine "
                            "dependencies: %s", self, e)
            raise EngineError("Failed to import Wenet engine dependencies.")
        ...
        required_wag_version = Version(self._required_kag_version)
        kag_version = Version(wenet_active_grammar.__version__)
        if not ((kag_version >= required_wag_version) and (kag_version.release[0:2] == required_wag_version.release[0:2])):
            self._log.error("%s: Incompatible wenet_active_grammar version %s! Expected ~= %s!" % (self, kag_version, required_wag_version))
            self._log.error("See https://dragonfly2.readthedocs.io/en/latest/wenet_engine.html#updating-to-a-new-version")
            if not os.environ.get('DRAGONFLY_DEVELOP'):
                raise EngineError("Incompatible wenet_active_grammar version")
        ...
        # Setup
        self._reset_state()
        self._recognition_observer_manager = WenetRecObsManager(self)
    ...
    def _load_grammar(self, grammar):
        ...
        wenet_rule_by_rule_dict = self._compiler.compile_grammar(grammar, self)
        wrapper = GrammarWrapper(grammar, wenet_rule_by_rule_dict, self,
    ...
    def _unload_grammar(self, grammar, wrapper):
        ...
        def unload():
            rules = list(wrapper.wenet_rule_by_rule_dict.keys())
    ...
    def activate_rule(self, rule, grammar):
        ...
        self._compiler.wenet_rule_by_rule_dict[rule].active = True

    def deactivate_rule(self, rule, grammar):
        """ Deactivate the given *rule*. """
        self._log.debug("Deactivating rule %s in grammar %s." % (rule.name, grammar.name))
        self._compiler.wenet_rule_by_rule_dict[rule].active = False
    ...
    def prepare_for_recognition(self):
        ...
        except WenetError as e:
            if len(e.args) >= 2 and isinstance(e.args[1], WenetRule):
                wenet_rule = e.args[1]
                raise self._compiler.make_compiler_error_for_wenet_rule(wenet_rule)
            raise
            ...
            while (not self._deferred_disconnect) and ((not end_time) or (time.time() < end_time)):
                ...
                if block is False:
                    ...
                    if not self._in_phrase:
                        ...
                        # Ongoing phrase
                        wenet_rules_activity = None
                    self._decoder.decode(block, False, wenet_rules_activity)
                    if self.audio_store:
                    ...
                    wenet_rule, words, words_are_dictation_mask, in_dictation = self._compiler.parse_partial_output(output)
                    in_complex = bool(in_dictation or (wenet_rule and wenet_rule.is_complex))
                    ...
                    if not self._ignore_current_phrase:
                        ...
                        is_acceptable_recognition = recognition.wenet_rule and (recognition.has_dictation or not (
                        ...
                        wenet_rule, parsed_output = recognition.wenet_rule, recognition.parsed_output
                        self._log.log(15, "End of phrase: eer=%.2f conf=%.2f%s, rule %s, %r",
                            expected_error_rate, confidence, (" [BAD]" if not is_acceptable_recognition else ""), wenet_rule, parsed_output)
                        ...
                        if self.audio_store:
                            if wenet_rule and is_acceptable_recognition:  # Don't store audio/metadata for bad recognitions
                                self.audio_store.finalize(parsed_output,
                                    wenet_rule.parent_grammar.name, wenet_rule.parent_rule.name,
    ...
    def _compute_wenet_rules_activity(self, phrase_start=True):
        window_info = {}
        if phrase_start:
        ...
        self._active_wenet_rules = set()
        self._wenet_rules_activity = [False] * self._compiler.num_wenet_rules
        for grammar_wrapper in self._iter_all_grammar_wrappers_dynamically():
            if grammar_wrapper.active and (not self._any_exclusive_grammars or grammar_wrapper.exclusive):
                for wenet_rule in grammar_wrapper.wenet_rule_by_rule_dict.values():
                    if wenet_rule.active:
                        self._active_wenet_rules.add(wenet_rule)
                        self._wenet_rules_activity[wenet_rule.id] = True
        self._log.debug("active wenet_rules (from window %s): %s", window_info, [kr.name for kr in self._active_wenet_rules])
        return self._wenet_rules_activity

    def _parse_recognition(self, output, mimic=False):
        if mimic or self._compiler.parsing_framework == 'text':
            with debug_timer(self._log.debug, "wenet_rule parse time"):
                detect_ambiguity = False
                results = []
                for wenet_rule in sorted(self._active_wenet_rules, key=lambda kr: 100 if kr.has_dictation else 0):
                    self._log.debug("attempting to parse %r with %s", output, wenet_rule)
                    words = self._compiler.parse_output_for_rule(wenet_rule, output)
                    if words is None:
                        continue
                    # self._log.debug("success %d", wenet_rule_id)
                    # Pass (wenet_rule, words) to below.
                    results.append((wenet_rule, words))
                    if not detect_ambiguity:
                        break

                if not results:
                    if not mimic:
                        # We should never receive an unparsable recognition from wenet, only from mimic
            ...
            wenet_rule, words, words_are_dictation_mask = self._compiler.parse_output(output,
                dictation_info_func=lambda: (self.audio_store.current_audio_data, self._decoder.get_word_align(output)))
            if wenet_rule is None:
                if words:
                    # We should never receive an unparsable recognition from wenet, unless it's empty (from noise)
                    self._log.error("unable to parse recognition: %r" % output)
                return Recognition.construct_empty(self)
        ...
        return Recognition(self, wenet_rule=wenet_rule, words=words, words_are_dictation_mask=words_are_dictation_mask)
...
class Recognition(object):
    """
    Wenet recognition results class.
    """

    def __init__(self, engine, wenet_rule, words, words_are_dictation_mask=None):
        assert isinstance(engine, WenetEngine)
        self.engine = engine
        self.wenet_rule = wenet_rule
        ...
        assert ((self.wenet_rule and self.words and self.words_are_dictation_mask)
            or (not self.wenet_rule and not self.words and not self.words_are_dictation_mask))
    ...
    def construct_empty(cls, engine):
        return cls(engine, wenet_rule=None, words=())
    ...
    def process(self, expected_error_rate=None, confidence=None, mimic=False):
        ...
        grammar_wrapper = grammar_wrappers[id(self.wenet_rule.parent_grammar)]
...
class GrammarWrapper(GrammarWrapperBase):
    ...
    def __init__(self, grammar, wenet_rule_by_rule_dict, engine,
                 recobs_manager):
        GrammarWrapperBase.__init__(self, grammar, engine, recobs_manager)
        self.wenet_rule_by_rule_dict = wenet_rule_by_rule_dict
    ...
    def recognition_callback(self, recognition):
        ...
        rule = recognition.wenet_rule.parent_rule
        words_are_dictation_mask = recognition.words_are_dictation_mask
        try:
            assert (rule.active and rule.exported), "Wenet engine should only ever return the correct rule"
