...
"""
Dictation container for the Wenet engine.
...
user_dictation_list = List('__wenet_user_dictation_list', [])
user_dictation_dictlist = DictList('__wenet_user_dictation_dictlist', {})

class _UserDictationSequenceRule(CompoundRule):
    spec = "<__wenet_user_dictation_sequence>"
    extras = [
        Repetition(Alternative([
            BaseDictation(format=False),
            ListRef('__wenet_user_dictation_listref', user_dictation_list),
            DictListRef('__wenet_user_dictation_dictlistref', user_dictation_dictlist),
        ]), min=1, max=16, name="__wenet_user_dictation_sequence"),
    ]
    exported = False
    def value(self, node):
